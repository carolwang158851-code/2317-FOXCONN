        ...
        if analysis.event_type == "QUARTERLY_EARNINGS":
            ...
            charts = [
                ...
                ChartData(
                    ...
                    title_zh="估值情境矩陣",
                    decision_question="財報公布前收盤價在EPS、P/B與股息研究情境下位於何處？",
                    ...
                    commentary_zh=[f"發現：鴻海官方2025Q4 basic EPS {valuation['q4_2025Eps']['value']}元，推導TTM EPS為{valuation['ttmEps']['value']}元、財報前P/E約{valuation['ttmPe']['value']}倍。", f"解釋：P/B使用最新直接揭露BVPS {valuation['governedBvps']['value']}元（{valuation['governedBvps']['period']}）；情境不是價格目標。", "企業價值含義：P/B需與ROE及股東權益複利共同判讀。"],
                    observation_zh=f"財報公布前收盤價{valuation['price']['value']}元（{valuation['price']['date']}）、P/B {valuation['pb']['value']}倍、推導TTM EPS {valuation['ttmEps']['value']}元、估算P/S {valuation['ps']['value']}倍。",
