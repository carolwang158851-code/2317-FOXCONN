        ...
        if analysis.event_type == "QUARTERLY_EARNINGS":
            ...
            charts = [
                ...
                ChartData(
                    ...
                    title_zh="2026H1獲利轉現金",
                    decision_question="2026H1獲利是否轉成營運現金與自由現金流？",
                    period="2026H1（官方累計值）",
                    source_evidence_ids=[*official_ids, cash_id],
                    labels=["2026H1"],
                    series=[
                        ChartSeries(label_zh="營業現金流", unit="新台幣百萬元", values=[analytics["h1OperatingCashFlowMillionTwd"]]),
                        ChartSeries(label_zh="資本支出", unit="新台幣百萬元", values=[analytics["h1CapexMillionTwd"]]),
                        ChartSeries(label_zh="自由現金流", unit="新台幣百萬元", values=[analytics["h1FreeCashFlowMillionTwd"]]),
                    ],
                    commentary_zh=[f"發現：官方2026H1 CFO為{analytics['h1OperatingCashFlowMillionTwd']}百萬元、Capex為{analytics['h1CapexMillionTwd']}百萬元、FCF為{analytics['h1FreeCashFlowMillionTwd']}百萬元。", "期間規則：這些數字是H1累計值，不是Q2單季現金流。", "退休任務含義：獲利與現金背離擴大，股利能力不得因EPS成長而上修。"],
                    observation_zh="2026H1營業現金流與自由現金流均為負。",
                ...
                ChartData(
                    ...
                    title_zh="2026H1資本支出",
                    decision_question="上半年規模成長使用多少資本支出？",
                    period="2026H1（官方累計值）",
                    ...
                    commentary_zh=[f"發現：官方2026H1 Capex為{analytics['h1CapexMillionTwd']}百萬元。", "限制：H1累計Capex不可直接除以Q2單季營收或營業利益。", "企業價值含義：自由現金流同時承受CFO負值與資本支出。"],
                    observation_zh="2026H1資本支出為官方累計值。",
                    interpretation_zh="不以期間不一致的分母判定資本支出強度。",
                ...
                ChartData(
                    ...
                    commentary_zh=[f"最新有效ROIC季度為{history.get('latestValidRoicQuarter', '2026Q1')}。", "限制：FY2026 Q2沒有治理合格的同口徑值；圖中缺口不是0，也不是下降。", "企業價值含義：歷史資本效率提供脈絡，但Q2改善仍未證實。"],
                ...
                ChartData(
                    ...
                    period=f"BVPS／ROE TTM {periods[0]}至{history.get('latestValidRoeQuarter', periods[-1])}",
                    source_evidence_ids=[master_id, *official_ids],
                    labels=history["periods"],
                    series=[
                ...
                ChartData(
                    ...
                    observation_zh="價值傳導鏈已推進到2026H1官方CFO與FCF，但現金結果為負；Q2資本報酬仍待驗。",
