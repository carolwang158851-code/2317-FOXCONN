    ...
    def _build_quarterly(
        ...
        *,
        ...
        sections = [
            ...
            self._section("EARNINGS_TO_CASH_QUALITY", "Q2獲利轉現金品質", f"官方2026H1 CFO為{analytics['h1OperatingCashFlowMillionTwd']}百萬元、Capex為{analytics['h1CapexMillionTwd']}百萬元、FCF為{analytics['h1FreeCashFlowMillionTwd']}百萬元；此處不以H1現金流除以Q2單季歸母淨利，也不把H1累計值改標為Q2單季。另以同口徑官方2026H1減2026Q1推導Q2 CFO為{analytics['q2StandaloneCfoMillionTwd']}百萬元、Capex為{analytics['q2StandaloneCapexMillionTwd']}百萬元、FCF為{analytics['q2StandaloneFcfMillionTwd']}百萬元；兩條FCF推導路徑差{analytics['q2StandaloneFcfRoundingDifferenceMillionTwd']}百萬元。因合併CFO與歸屬母公司淨利的期間與會計範圍不同，本期只標示獲利—現金背離，不計算或展示同口徑標準現金轉化率。", "MIXED", official_ids + [cash_id]),
            ...
            self._section("RETIREMENT_CASHFLOW_IMPLICATION", "治理、企業價值與退休任務總結", f"Q2營收、營業利益、歸母淨利、EPS與BVPS已由正式來源升格；{roe['currentPeriod']} ROE {roe['currentPct']}%為官方未年化值。Q2沒有證明AI專屬營收占比或利潤率，Cloud & Networking 51%不得稱為AI占比；Q2同口徑ROIC仍待標準化NOPAT與完整平均投入資本，資料缺口不得解讀為零或趨勢。2026H1 CFO與FCF為負，現金回收仍是主要驗證點。核心論點存續，但估值安全、退休現金流安全與股利能力都不因本季成長自動上修。", "INFERENCE", official_ids + [master_id, cash_id]),
