...
def build_full_history_charts(package_root: Path, analysis: AnalysisPacket, historical_baseline: Mapping[str, Any] | None = None) -> tuple[list[ChartData], list[dict[str, Any]], list[str]]:
    ...
    charts = [
        ...
        ], full_sources, ["所有可比季度與資料點均保留，改採首期=100避免營收尺度壓平毛利與營業利益。", "Q2營業利益增幅快於營收，但仍須檢查現金與資本效率。"]),
        ...
        ], full_sources, ["毛利率與營益率分開呈現且保留完整季度。", "營益率改善不能被解讀為毛利率同步改善。"]),
        ...
        ], full_sources, ["EPS保留所有可比季度。", "每股價值仍須與股數、BVPS及FCF共同驗證。"]),
    ]
    ...
    ], [cash_source, *sources], ["正式現金流可比期與Q2同口徑推導均保留。", "Q2負FCF使結構性風險尚未排除，仍需後續現金回收驗證。"] ))
    ...
    if cumulative_labels:
        ...
        ], sources, ["依年度與累計期間基礎排列；FY、H1與M9不以連續等距時間序列連線。", "本機基線已包含FY2024、2025H1、2025M9、2025FY與2026H1官方累計資料。"], visualization_type="EVIDENCE_TABLE" ))
    ...
    ], sources, ["現有三個同口徑觀察期全數保留。", "金額占用上升而CCC改善，較支持成長吸收，但不是現金已回收。"] ))
    ...
    ], sources, ["NWC Proxy固定為應收帳款加存貨減應付帳款。", "這不是公司正式揭露的完整淨營運資金。"] ))
    charts.extend([
        _chart("full_history_roic", "ROIC完整歷史與本期待驗", "資本效率是否改善？", labels, [ChartSeries(label_zh="ROIC", unit="%", values=[r["ROIC_Precise_Pct"] for r in rows] + ["INSUFFICIENT_DATA"])], full_sources, ["歷史ROIC全部保留；Q2缺口不是零。", "Q2單季同口徑ROIC待補，不以第三方TTM值替代。"]),
        _chart("full_history_bvps", "每股淨值完整歷史與本期待驗", "帳面價值是否持續傳達至每股？", labels, [ChartSeries(label_zh="BVPS", unit="新台幣元", values=[r["BVPS"] for r in rows] + ["INSUFFICIENT_DATA"])], full_sources, ["歷史BVPS全部保留；Q2直接值尚未提供。", "每股價值需與EPS、FCF及股數共同驗證。"]),
        _chart("full_history_valuation", "P/B與TTM P/E完整歷史", "市場評價與股東資本效率如何變化？", periods, [
        ...
        ], [master_source], ["正式資料可得的P/B與TTM P/E季度觀察全部保留。", "估值為描述性歷史，不形成交易門檻。"]),
    ])
    ps_rows = observations_for(historical_baseline, "PS_TTM")
    charts.append(_chart("full_history_ps", "TTM P/S可比歷史", "市值相對過去十二個月營收如何變化？", [item["period"] for item in ps_rows], [
        ChartSeries(label_zh="TTM P/S", unit="倍", values=[item["value"] for item in ps_rows]),
    ], [master_source], ["P/S只從四季營收齊備的2021Q4開始。", "不以最新TTM營收回套過去市值，避免前視偏誤。"] ))
    ...
    unavailable = ["PERIOD_END_SHARE_COUNT:僅有外部交叉核對，未升格正式值", "PEER_VALUATION_CAPITAL_EFFICIENCY_HISTORY:無可驗證歷史快照"]
...
def compile_existing_phaseb1_result(*, package_root: Path, analysis: AnalysisPacket, evidence: Any, trigger_context: Mapping[str, Any], output_root: Path, previous_state: Mapping[str, Any] | None = None, source_mother: Path | None = None) -> dict[str, Any]:
    ...
    chapters = _build_chapters(
        ...
        historical_baseline, forward, rule_result,
    )
    rendered = render_owner_review_candidate(
        report_title="鴻海企業價值戰報", headline="營運槓桿轉強，現金與資本效率仍待驗證",
        deck="本季獲利增速快於營收，但自由現金流仍為負；核心論點維持，新增資本門檻僅部分通過。",
