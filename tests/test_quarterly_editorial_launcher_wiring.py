...
class QuarterlyEditorialLauncherWiringTests(unittest.TestCase):
    def test_launcher_exposes_one_generic_import_apply_and_history_surface(self):
        ...
        self.assertIn("匯入版本 JSON", panel)
        self.assertIn("匯入戰報成品", panel)
        self.assertIn("只接受 .json", panel)
        self.assertIn("只接受 .html / .pdf", panel)
        self.assertIn("此入口只接受版本 JSON；HTML/PDF 請使用「匯入戰報成品」。", body)
        ...
        self.assertIn("/api/p1008/quarterly-editorial/artifact-import", body)
    ...
    def test_server_routes_are_whitelisted_and_report_cards_embed_versions(self):
        ...
        for route in ("catalog", "history", "import", "artifact-import", "apply"):
        ...
        self.assertIn("REPORT_ARTIFACT_REPORT_KEY_MISMATCH", server)
