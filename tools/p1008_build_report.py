...
def _analysis_run_id(package_root: Path) -> str:
    ...
    if root.is_dir():
        for path in root.glob("*/run_manifest.json"):
            try:
                ...
                continue
            if payload.get("state") == "ANALYSIS_CANDIDATE_READY":
...
def main() -> int:
    ...
    try:
        run_id = args.run_id or _analysis_run_id(package_root)
        result = PhaseB1Pipeline(package_root).build_report(run_id=run_id)
